...
public class SVAdjacentRegionWindowBuilder {
	 ...
	 */
	private Map<SimpleSVLocus, List<PiAndThetaWWindow>> svLeftSideWindowsMap;
	/**
	 * right windows, the one with the index=0 is the one closest to the end of the sv
	 */
	private Map<SimpleSVLocus, List<PiAndThetaWWindow>> svRightSideWindowsMap;
	
	////////////
	private Map<String,List<PiAndThetaWWindow>> chromBuiltWindowsMap;
	...
	void run() {
		...
		for(SimpleSVLocus locus:this.svLocusList) {
			...
			for(int i=0;i<this.windowNum;i++) {
				...
				if(start>0) {
					PiAndThetaWWindow leftWindow=new PiAndThetaWWindow(locus.getChrom(), start, end);
				...
				///right window
				start=locus.getEnd()+i*this.windowSize+1;
				end=start+this.windowSize;
				if(this.chromLenReader.getChromNameLengthMap().get(locus.getChrom())>=end) {
					PiAndThetaWWindow rightWindow=new PiAndThetaWWindow(locus.getChrom(), locus.getEnd()+i*this.windowSize, locus.getEnd()+(i+1)*this.windowSize);
	 ...
	 */
	public Map<SimpleSVLocus, List<PiAndThetaWWindow>> getSvLeftSideWindowsMap() {
	 ...
	 */
	public Map<SimpleSVLocus, List<PiAndThetaWWindow>> getSvRightSideWindowsMap() {
