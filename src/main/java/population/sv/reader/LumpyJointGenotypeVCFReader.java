...
/**
 * reader for a vcf file containing jointly called SVs for a population by merging lumpy generated single sample genotyped sv generated by SURVIVOR
 * 
 * 
 * 
 * ===============================
 * 1. all sample genotype are not available ('./.')
 * 
 * 2. types of SVs
 * 		1. DEL
 * 		2. DUP : tandem duplication
 * 			
 * 		3. INV
 * 		4. INS : but not found??
 * 		5. CNV : but not found???
 ...
 *
 */
public class LumpyJointGenotypeVCFReader {
	...
	public LumpyJointGenotypeVCFReader(
			Path vcfFile, 
...
//				System.out.println("skip sv type:"+SVTYPE);
			...
			for(int i=0;i<vc.getNSamples();i++) {
				if(vc.getGenotype(i).isNoCall()) {
					sampleIndexGenotypeMap.put(i, Genotype.MISSING);
				}else {
